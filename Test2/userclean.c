...
int main(){
    ...
    int verify=scanf("%s",original_username);

    if (verify <= 0){
        printf("Something Wrong happened with SCANF\n");
        return 1;
    }

    printf("I received this: >>%s<<",original_username);
...
int obtain_username(char* username){
    ...
    printf("namesize: %d, strlen: %ld",name_size,strlen(username));
    return name_size;
}
